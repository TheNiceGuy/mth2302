...
\begin{theoreme}
	$f_X(x)=\dfrac{\d{}}{\d{x}}F_X(x)$.
\end{theoreme}

\begin{theoreme}
	$F_X(x)=\displaystyle\int_{-\infty}^xf_X(t)\d{t}$.
\end{theoreme}

\begin{theoreme}\label{th:masse_repartition}
	$F_X(x)=\displaystyle\sum_{x_k\leq x}P_X(x_k)$.
\end{theoreme}
...
\begin{exemple}
	...
	\begin{equation*}
		F_X(k)=\left\{
			\begin{matrix}
				\SI{0.0}{}, & \phantom{0\;\leq}x<0\\
				\SI{0.7}{}, & 0\leq x<1\\
				\SI{0.8}{}, & 1\leq x<2\\
				\SI{0.9}{}, & 2\leq x<3\\
				\SI{1.0}{}, & 3\leq x\phantom{<0\;}\\
			\end{matrix}
		\right.
	\end{equation*}
\end{exemple}
...
\subsection{Fonctions conditionnelles}
\begin{definition}
	Une \textit{fonction conditionnelle} $F_X(x|A)$ est la probabilité qu'une
	variable aléatoire $X$ soit inférieure à $x$ sachant un évènement $A$.
\end{definition}


\begin{equation*}
	F_X(x|A)=P(X\leq x|A)=\frac{P(\left\{X\leq x\right\}\cap A)}{P(A)}
\end{equation*}

\begin{equation*}
	P_X(x_k|A)=P(X=x_k|A)=\frac{P(\left\{X=x_k\right\}\cap A)}{P(A)}=
	\left\{
		\begin{matrix}
